...
const PILLARS = ["Access", "Awareness", "Amenities"];
export type StoryMetric = "visitors" | "gap" | "bottleneck";

function Step({ n, kicker, color, cue, active, last, onClick, children }: {
  n: number; kicker: string; color: string; cue: string; active?: boolean; last?: boolean; onClick: () => void; children: React.ReactNode;
}) {
  return (
    <motion.div className="relative min-w-0" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 * (n - 1), duration: 0.5, ease: EASE }}>
...
export function Story({ metric, onMetric }: { metric: string; onMetric: (m: StoryMetric) => void }) {
  ...
  const s = useMemo(() => {
    ...
    const room = capacity.filter((c) => untapped.includes(c.code)).reduce((a, c) => a + c.max_extra_visitors_k, 0);
    ...
    return {
      total, untapped, room, held, dests, origin, sim,
  ...
  }, [rows, gap, pillars, capacity, assumptions]);

  const names = (codes: string[]) => codes.map((c) => STATE_LABEL[c]).join(", ").replace(/, ([^,]*)$/, " and $1");
  const giniChange = (s.sim.concentration_after.gini / s.sim.concentration_before.gini - 1) * 100;

  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Step n={1} kicker="The problem" color="#e66767" cue="Show crowds" active={metric === "visitors"} onClick={() => onMetric("visitors")}>
        ...
        <div className="mt-auto pt-3">
          <div className="flex h-2 gap-px overflow-hidden rounded-full">
            {s.shares.map((x, i) => (
              <motion.span key={x.code} title={`${STATE_LABEL[x.code]} ${fmt.pct(x.share * 100)}`} className="h-full origin-left" style={{ width: `${x.share * 100}%`, background: i < 3 ? "#e66767" : "var(--slate-6)" }}
                initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.2 + 0.02 * i, duration: 0.4, ease: EASE }} />
            ))}
          </div>
          <p className="mt-1.5 truncate text-[11px] text-[#f0a3a3]">{names(s.shares.slice(0, 3).map((x) => x.code))}</p>
        </div>
      </Step>
      ...
      <Step n={2} kicker="The opportunity" color="#3987e5" cue="Show potential" active={metric === "gap"} onClick={() => onMetric("gap")}>
        ...
        <div className="mt-auto flex flex-wrap gap-1 pt-3">
          {s.untapped.map((c) => (
            <button key={c} onClick={(e) => { e.stopPropagation(); setSelected(c); onMetric("gap"); }}
      ...
      <Step n={3} kicker="The obstacle" color="#d9a326" cue="Show bottlenecks" active={metric === "bottleneck"} onClick={() => onMetric("bottleneck")}>
      ...
      <Step n={4} kicker="The payoff" color="#199e70" cue="Open simulator" last onClick={() => router.push("/simulator")}>
        ...
        <div className="mt-auto flex flex-wrap gap-x-4 gap-y-1 pt-3 text-[11px] text-muted-foreground">
          <span>Concentration <b className="font-semibold text-[#5fd0a5]">{fmt.signed(giniChange, 1)}%</b></span>
          <span>National spending <b className={cn("font-semibold", s.sim.net_national_rm_m >= 0 ? "text-[#5fd0a5]" : "text-[#f0a3a3]")}>{s.sim.net_national_rm_m >= 0 ? "+" : "−"}{fmt.rmM(Math.abs(s.sim.net_national_rm_m))}</b></span>
        </div>
      </Step>
    </div>
  );
